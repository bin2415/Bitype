...
llvm::cl::opt<bool> ClBitypeDynamicCast(
  "bitype-dynamic-change",
  llvm::cl::desc(
    "Change the dynamic cast with the bitype stype"
...
//Bitype substituted dynamic_cast function
static llvm::Constant *getItaniumBitypeDynamicCastFn(CodeGenFunction &CGF){
    llvm::Type *Int8PtrTy = CGF.Int8PtrTy;
    llvm::Type *Int32Ty = CGF.Int32Ty;
    llvm::Type *Int8Ty = CGF.Int8Ty;
    
    llvm::Type *Args[3] = {Int8PtrTy, Int32Ty, Int8Ty};
    ...
    return CGF.CGM.CreateRuntimeFunction(FTy, "__bitype_dynamic_cast_verification");
}
...
llvm::Value *ItaniumCXXABI::EmitDynamicCastCall(
  ...
  // substitute the dynamic cast
  if((ClBitypeDynamicCast) && CGF.SanOpts.has(SanitizerKind::Bitype)){
    //llvm::errs() << "In dynamic\n";
    ...
    if(ClassTy){
      ...
      if(!ClassDecl || !ClassDecl->isCompleteDefinition() ||
        ...
        std::string className = ClassDecl->getName().str();
        std::map<std::string, unsigned> RS = CGF.CGM.getSanitizerRelationSet();
        std::map<std::string, unsigned>::iterator rsIter;
        rsIter = RS.find(className);
        llvm::errs() << "dynamic cast related class is" << className << "\n";
        //if(rsIter == RS.end()){
        ...
        //CGF.BitypeEmitCheck("__bitype_verify_cast", Address, byteIndex, safeCode);
        llvm::Value *IndexValue = llvm::ConstantInt::get(CGF.Int32Ty, byteIndex);
        llvm::Value *OffsetValue = llvm::ConstantInt::get(CGF.Int8Ty, safeCode);

        llvm::Value *ArgsBitype[] = {Value, IndexValue, OffsetValue};
        DcastResult = CGF.EmitNounwindRuntimeCall(
            getItaniumBitypeDynamicCastFn(CGF), ArgsBitype);
        Value = CGF.Builder.CreateBitCast(DcastResult, DestLTy);
        if(DestTy->isReferenceType()){
          llvm::BasicBlock *BadCastBlock = 
            CGF.createBasicBlock("dynamic_cast_bitype.bad_cast");
