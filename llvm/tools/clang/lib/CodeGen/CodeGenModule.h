...
private:
  ...
  std::unique_ptr<std::map<std::string, unsigned> > SanitizerIndex;
  std::unique_ptr<std::map<std::string, unsigned> > SanitizerDebug;
...
public:
  ...
  const std::map<std::string, unsigned> &getSanitizerClassIndexSet() const{
    return *SanitizerIndex;
  }

  const std::map<std::string, unsigned> &getSanitizerDebugInfo() const{
