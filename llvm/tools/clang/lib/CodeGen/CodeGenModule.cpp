...
static llvm::cl::opt<bool> LimitedCoverage(
    ...
    llvm::cl::init(false));
static llvm::cl::opt<std::string> ClBitypeDebugFile(
  "bitype-debug-file",
  llvm::cl::desc("the file path of debug file path"),
  llvm::cl::Hidden, llvm::cl::init("downcastLoc.txt"));
...
CodeGenModule::CodeGenModule(ASTContext &C, const HeaderSearchOptions &HSO,
  ...
  if (CodeGenOpts.CoverageMapping)
   ...
   if(LangOpts.Sanitize.has(SanitizerKind::Bitype)){
    llvm::BitypeGetConfig bitypeUtils;
    SanitizerRelation sr;
    SanitizerRS = sr.createCodeMap(bitypeUtils.getCodeMapPath());
    //llvm::errs() << "code map variable is " << bitypeUtils.getCodeMapPath() << "\n";
    SanitizerIndex = sr.getClassIndex(bitypeUtils.getCastRelatedPath());

    SanitizerDebug = sr.getDebugInfo(ClBitypeDebugFile);


    //llvm::errs() << "cast related path is " << bitypeUtils.getCastRelatedPath() << "\n";
    //llvm::errs() << "I have got the class set, the set size is " << SanitizerIndex->size() << "\n";
    // for(std::pair<std::string, unsigned> codeSet : *SanitizerRS){
    //   llvm::errs() << "Class Name " << codeSet.first << " : " << codeSet.second << "\n";
    // }
  }
}
