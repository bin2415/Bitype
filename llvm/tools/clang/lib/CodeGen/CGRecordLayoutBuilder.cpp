...
/*void CGRecordLowering::accumulateBTCPtrs(){
  if(Layout.hasOwnBTCPtr()){
    //llvm::errs() << "Happy!!!! I have enter the accumulateBTCPtrs, one step to success\n";
    CharUnits offset = CharUnits::Zero();
    if(Layout.hasOwnVFPtr()){
      offset = Context.toCharUnitsFromBits(Context.getTargetInfo().getPointerWidth(0));
    }
    Members.push_back(MemberInfo(offset, MemberInfo::BTCPtr,
        llvm::Type::getInt32PtrTy(Types.getLLVMContext())));
  }
}*/
