...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_BitCast: {
    ...
    if(ClHandleReinterpretCast && CGF.SanOpts.has(SanitizerKind::Bitype)){
      if(isa<CXXReinterpretCastExpr>(CE)){
        if(auto PT = DestTy->getAs<PointerType>()){
          CGF.EmitBitypeReinterpretCast(PT->getPointeeType(), Src, true);
        }
      }
    }
  ...
  case CK_BaseToDerived: {
    ...
    if(CGF.SanOpts.has(SanitizerKind::Bitype)){
      //llvm::errs() << "do not have the bitype ?\n";
      llvm::Value *NonVirtualOffset = 
        CGF.CGM.GetNonVirtualBaseClassOffset(DerivedClassDecl, CE->path_begin(), CE->path_end());
      llvm::Value *Address;
      llvm::Value *Address1 = Base.getPointer();
      if(!NonVirtualOffset){
          Address = Base.getPointer();
      }else{
          Address = Derived.getPointer();
      }
      QualType DerivedTy = DestTy->getPointeeType();
      const RecordType *DeClassTy = DerivedTy->getAs<RecordType>();
      //llvm::errs() << "Kidding me?\n";
      if(DeClassTy){
        ...
        if(DeClassDecl && DeClassDecl->isCompleteDefinition() &&
          DeClassDecl->hasDefinition() && !DeClassDecl->isAnonymousStructOrUnion()){
            ...
            //llvm::errs() << "Derived class name" << DeClassDecl->getName() << "\n";
            std::string DeclClassName = DeClassDecl->getName().str();
            std::map<std::string, unsigned> RS = CGF.CGM.getSanitizerRelationSet();
            std::map<std::string, unsigned>::iterator rsIter;
            rsIter = RS.find(DeclClassName);
            assert(rsIter != RS.end() && "Error, the cast class set is incomplete!");
            if(rsIter != RS.end()){
              // std::map<std::string, unsigned> classAlign = CGF.getContext().getSanitizerAlignment();
              // int alignment = classAlign.find(DeclClassName)->second;
              unsigned codeN = rsIter->second - 1;
              unsigned byteIndex = codeN / 8;
              unsigned byteOffset = codeN % 8;
              unsigned char safeCode = 1 << byteOffset;
              std::map<std::string, unsigned> tempDebug = CGF.CGM.getSanitizerDebugInfo();
              int debugIndex = -1;

              PresumedLoc PLoc = CGF.getContext().getSourceManager().getPresumedLoc(CE->getLocStart());
              if(PLoc.isValid()){
              ...
              CGF.BitypeEmitCheck("__bitype_cast_verification", Address1, Address, byteIndex, safeCode, debugIndex);
            }
          }
      }
    }
