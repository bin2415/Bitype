...
llvm::cl::opt<bool> ClHandlePlacementNew("handle-placement-new",
                                        llvm::cl::desc("handle placement new"),
...
void
CodeGenFunction::BitypeEmitObjTraceInst(const CXXRecordDecl *ClassDecl, llvm::Value *ValueAddr,
                                        uint64_t offsetInt, char *InstName){
  ...
  std::map<std::string, unsigned> classIndexSet = CGM.getSanitizerClassIndexSet();
  std::map<std::string, unsigned>::iterator findClass = classIndexSet.find(DstTyStr);
  //llvm::errs() << "Handle placement structName in clang is " << DstTyStr << "\n";
  if(findClass != classIndexSet.end()){
    llvm::Value *Offset = llvm::ConstantInt::get(CGM.Int64Ty, offsetInt);
    llvm::Value *classIndex = llvm::ConstantInt::get(CGM.Int32Ty, findClass->second);
    ...
    llvm::FunctionType *FnType = llvm::FunctionType::get(CGM.VoidTy, ArgTypes, false);

    llvm::Value *Fn = CGM.CreateRuntimeFunction(FnType, InstName);
