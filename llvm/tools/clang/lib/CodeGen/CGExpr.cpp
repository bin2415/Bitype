...
void CodeGenFunction::BitypeEmitCheck(StringRef FunName, llvm::Value* Address1, llvm::Value* Address,
      unsigned index, unsigned char safeCode, int debugIndex){
        //llvm::errs() << "coming in ?\n";
        bool blacklisted =
          CGM.getContext().getSanitizerBlacklist().isBlacklistedFunction(
          SanitizerKind::Bitype, CurFn->getName()) ? 1 : 0;
        ...
        Args.push_back(EmitCheckValue(Address1));
        ArgTypes.push_back(IntPtrTy);
        Args.push_back(EmitCheckValue(Address));
        ArgTypes.push_back(IntPtrTy);
        llvm::Type* int32tya = Builder.getInt32Ty();
        llvm::Type* int8tya = Builder.getInt8Ty();
        llvm::Value *indexValue = llvm::ConstantInt::get(int32tya, index);
        llvm::Value *offsetValue = llvm::ConstantInt::get(int8tya, safeCode);
        Args.push_back(indexValue);
        Args.push_back(offsetValue);
        ArgTypes.push_back(int32tya);
        ArgTypes.push_back(int8tya);
        llvm::Value* debugValue = llvm::ConstantInt::get(int32tya, debugIndex);
        Args.push_back(debugValue);
        ArgTypes.push_back(int32tya);
...
LValue CodeGenFunction::EmitCastLValue(const CastExpr *E) {
  ...
  case CK_BaseToDerived: {
    ...
    //Added Bitype
    //llvm::errs() << "Base to Derived\n";
    if(SanOpts.has(SanitizerKind::Bitype)){
      //llvm::errs() << "do not have the bitype ?\n";
      llvm::Value *NonVirtualOffset = 
        CGM.GetNonVirtualBaseClassOffset(DerivedClassDecl, E->path_begin(), E->path_end());
      llvm::Value *Address;
      llvm::Value *Address1 = LV.getAddress().getPointer();
      if(!NonVirtualOffset){
          Address = LV.getAddress().getPointer();
      }else{
          Address = Derived.getPointer();
      }
      QualType DerivedTy = E->getType();
      const RecordType *DeClassTy = DerivedTy->getAs<RecordType>();
      if(DeClassTy){
        const CXXRecordDecl * DeClassDecl = cast<CXXRecordDecl>(DeClassTy->getDecl());
        if(DeClassDecl && DeClassDecl->isCompleteDefinition() &&
          DeClassDecl->hasDefinition() && !DeClassDecl->isAnonymousStructOrUnion()){
            // llvm::errs() << "Current class is " << DeClassDecl->getName() << "\n";
            // for(auto &I : DeClassDecl->bases()){
            ...
            //   if(!Base)
            //     continue;
            //   llvm::errs() << "base class is " << Base->getName() << "\n";
            // }
            std::string DeclClassName = DeClassDecl->getName().str();
            std::map<std::string, unsigned> RS = CGM.getSanitizerRelationSet();
            std::map<std::string, unsigned>::iterator rsIter;
            rsIter = RS.find(DeclClassName);
            if(rsIter != RS.end()){
              unsigned codeN = rsIter->second - 1;
              unsigned byteIndex = codeN / 8;
              unsigned byteOffset = codeN % 8;
              unsigned char safeCode = 1 << byteOffset;
              int debugIndex = -1;
              std::map<std::string, unsigned> tempDebug = CGM.getSanitizerDebugInfo();

              PresumedLoc PLoc = getContext().getSourceManager().getPresumedLoc(E->getLocStart());
              if(PLoc.isValid()){
                    //llvm::errs() << "DownCast occurs " << PLoc.getFilename() << " : " << PLoc.getLine() << "\n";
                    string tempLoc = PLoc.getFilename();
                    tempLoc += ":";
                    tempLoc += std::to_string(PLoc.getLine());
                    std::map<std::string, unsigned>::iterator tempDebugIter = tempDebug.find(tempLoc);
                    if(tempDebugIter != tempDebug.end())
                      {
                        debugIndex = tempDebugIter->second;
                        //llvm::errs() << "get the debug code " << debugIndex << "\n";
                      }
                      // else{
                      //   llvm::errs() << "sorry, can't get the debug info " << tempLoc << "\n";
                      // }
              }
              if(debugIndex == -1){
                llvm::errs() << "Sorry, don't get the debug code\n";
              }
              BitypeEmitCheck("__bitype_cast_verification", Address1, Address, byteIndex, safeCode, debugIndex);
            }
