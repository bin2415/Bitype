...
SanitizerArgs::SanitizerArgs(const ToolChain &TC,
  ...
  // parse the Bitype relationship files

  // if(Kinds & Bitype){
  //   bool findPath = false;
  //   for(const auto *Arg : Args){
  //     if(Arg->getOption().matches(options::OPT_fsanitize_relation)){
  //       Arg->claim();
  //       std::string BLPath = Arg->getValue();
  //       if(llvm::sys::fs::exists(BLPath)){
  //         RelationFileStr = BLPath;
  //         findPath = true;
  //         //llvm::errs() << "I find the fsanitize-relation file location\n";
  //       }else{
  //         D.Diag(clang::diag::err_drv_no_such_file) << BLPath;
  //       }
  //     }//end if(Arg->getOption())
  //   } //end for

  //   if(!findPath){
  //     if(llvm::sys::fs::exists("coding-num.txt")){
  //       RelationFileStr = "coding-num.txt";
  //     }else{
  //       D.Diag(clang::diag::err_drv_no_such_file) << "relationship-set.txt. Please config the class inherience relation with option -fsanitize_relation=<file location>";
  //     }
  //   }
  // } //end if(Kinds & Bitype)
...
void SanitizerArgs::addArgs(const ToolChain &TC, const llvm::opt::ArgList &Args,
  ...
  // if (Sanitizers.has(Bitype)){
  //   CmdArgs.push_back(Args.MakeArgString("-fsanitize-relation=" + Twine(RelationFileStr)));
  // }
